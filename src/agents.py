...
class AgentConfig:
    def __init__(
        ...
        model_name: str = "gpt-4.1-mini", # local model path for huggingface도 여기서 입력
        api_type: Literal["openai", "anthropic", "huggingface"] = "openai",
...
class PersonaAgent:
    ...
    def __init__(self, config: AgentConfig):
        ...
        if config.api_type == "anthropic":
            self.llm = ChatAnthropic(model=config.model_name, temperature=config.temperature)
